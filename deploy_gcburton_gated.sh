...
# the gated docroot (see "Gated access" in HANDOFF.md, or ./deploy.sh, which the
# weekly refresh runs). This script is kept for a genuine nginx host.
...
✗ Refusing to run: Caddy is installed on this host, so this nginx recipe is
  ...
  dashboard is a file copy into the already-gated docroot — see "Gated access"
  in HANDOFF.md, or use ./deploy.sh (the weekly refresh already calls it).
