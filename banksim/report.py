...
def km1(position: CapitalPosition, period: PeriodResult | None = None) -> str:
    ...
    lines += [
        ...
        _row("Pillar 2A gross", fmt_pct(req.pillar2a_gross)),
        _row("  less SME lending adjustment", f"({fmt_pct(req.sme_lending_adjustment)})"),
        _row("  less infrastructure lending adjustment",
             f"({fmt_pct(req.infrastructure_lending_adjustment)})"),
